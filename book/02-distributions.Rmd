...
```{r circulare-day11, fig.width = 5, fig.height = 5}
rbcol <- c("red3", 
           "darkgoldenrod1", 
           "springgreen3", 
           "dodgerblue2", 
...
They don't give a data source beyond "Press reports; *The Economist*", but there aren't that many countries, so I can just create the data table myself, with the help of Wikipedia's article on  [legal recognition of non-binary gender](https://en.wikipedia.org/wiki/Legal_recognition_of_non-binary_gender).
...
nb_passports <- tribble(
  ...
  "United States", "Self-declaration", 2021,
...
```{r}
data_map <- left_join(world_wintri, data_na, 
                          by = c("gu_a3" = "code")) %>%
