...
I got this dataset from a class assignment that cites [Our World in Data](https://ourworldindata.org/population-sources), but I'm still not sure exactly where the data file came from, so you can <a href="data/disease-burden-vs-health-expenditure-per-capita.csv" download>get the file here</a>.
...
flower <- function(n = 6, petal_dist = 500, r = 250, rot = -pi/2,
  ...
  if (!is_null(petal_icon)) {
    icons <- map(petal_icon, readLines, warn=F) %>%
      map_chr(paste,collapse = "\n")
    
    petals_icons <- glue('    <g transform="translate({coords$x-(petal_text_size)},{coords$y-(petal_text_size)}) scale({petal_text_size/512})">{icons}</g>') %>%
    paste(collapse = "\n")
  } else {
    petals_icons <- ""
  }
  ...
  svg <- paste("<svg viewBox = '0 0 {{2*cx}} {{2*cy}}'>", 
               svg_style,
               center, center_texts,
               petal_fix1, petals, petal_fix2, petals_text, petals_icons,
               "</svg>",
               sep = "\n") %>%
    glue(.open = "{{", .close = "}}")
...
I still haven't got the code right for centering icons, but I figured out how to add SVG icons into the petals.

```{r}
svg <- flower(n = 5, petal_dist = 450, r = 300,
              petal_colors = c("#4864DB", "#10D4CF", "#8DEA74", "#F5C748", "#EA4E11"),
              petal_text = "",
              petal_icon = c('images/fontawesome/solid/code-compare.svg',
                             'images/fontawesome/regular/bell.svg',
                             'images/fontawesome/regular/handshake.svg',
                             'images/fontawesome/regular/calendar-days.svg',
                             'images/fontawesome/solid/question.svg'),
              petal_text_size = 100,
              center_color = "transparent",
              center_text = c("30-Day", "Chart", "Challenge"),
              center_text_offsets = c(-60, 10, 80),
...
I'm going back to data simulation. Someone asked me recently how to add a continuous factor to a mixed effects model. I'm going to build a function for simulating data with a nested structure (people within countries) and a continuous predictor of time (0-10). I'll plot the data with various values for the parameters, including random intercepts and *slopes*.
...
First, I need to write a function to generate the simulated data. I want to be able to vary the fixed and random effects parameters, but am setting almost everything to 0 as a default.
...
The default plot is pretty boring. Everyone's score is is equal to the grant intercept.
